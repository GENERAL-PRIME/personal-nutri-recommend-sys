...
from utils.user_registry import (
    ...
    save_user_profile, touch_last_run, list_users, save_last_plan, get_last_plan
)
...
def login_user():
    ...
    try:
        ...
        if not user_exists(user_id):
            return jsonify({"error":f"User ID '{user_id}' not found."}), 404
        record = get_user(user_id)
        last_plan = get_last_plan(user_id)
        return jsonify({"status":"ok","user":record,"last_plan":last_plan})
    except Exception as e:
        traceback.print_exc()
        return jsonify({"error":str(e)}), 500
...
def recommend():
    try:
        ...
        # Save plan locally for persistence
        if user_exists(uid) and uid != 'WEB_USER':
            save_last_plan(uid, plan_payload)
