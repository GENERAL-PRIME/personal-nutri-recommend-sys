...
def _sanitize_nan(obj):
    ...
    and convert non-JSON-serializable Mongo/date types to strings."""
...
def user_countdown(user_id):
    """Return real-time countdown for the user's active plan."""
    try:
        _require_mongo()
        u = mdb.users_col.find_one({"user_id": user_id.upper()})
        if not u:
            return jsonify({"error": "User not found"}), 404
        plan_start = u.get("plan_start")
        plan_weeks = u.get("plan_weeks", 8)
        if not plan_start:
...
def user_checkin():
    """
    User submits weekly progress: current weight, optionally waist/notes.
    Updates metrics and returns re-calculated countdown + projection.
    """
    try:
        _require_mongo()
        d      = request.get_json(force=True)
...
def before_after():
    """
    Called when user completes the plan and submits new measurements.
    Returns a detailed before-vs-after summary and readiness for new plan.
    """
    try:
        _require_mongo()
        d        = request.get_json(force=True)
        ...
        if new_bmi >= 25 and wt_change < 0:
            suggested_goal = "weight_loss" if new_bmi >= 25 else "maintain"
        elif new_bmi < 18.5 and wt_change > 0:
            suggested_goal = "weight_gain_mild" if new_bmi >= 17 else "weight_gain"
        elif new_bmi >= 18.5 and new_bmi < 25:
            suggested_goal = new_goal or "maintain"
        else:
            suggested_goal = new_goal or "maintain"
        ...
        summary = {
            ...
            "suggested_next_goal": suggested_goal,
            "ready_for_new_plan": True,
        }
...
def _before_after_message(wt_change: float, old_bmi: float, new_bmi: float) -> str:
    ...
    if wt_change < -5:
        ...
        return f"Gained {wt_change:.1f} kg. Keep consistent with your surplus and training."
