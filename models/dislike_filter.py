...
class DislikeFoodFilter:
    ...
    SPECIAL_RULES = {
        ...
        "pork": {
            "description": "No pork or pork-derived products",
            "remove_non_vegetarian": False,
            "remove_eggs": False,
            "name_keywords": [
                "pork",
                "bacon",
    ...
    def __init__(
        self, food_data_path=None, dislike_data_path=None, food_df=None, dislike_df=None
    ):
        ...
        if food_df is not None and dislike_df is not None:
            ...
            raise ValueError(
                "Provide either DataFrames or file paths to DislikeFoodFilter"
            )
        self._preprocess()
    ...
    def _prefix_mask(self, df: pd.DataFrame, prefixes: list) -> pd.Series:
        ...
        return df["food_id"].apply(lambda fid: any(fid.startswith(p) for p in prefixes))
    ...
    def _get_dislike_rules(self, dislike_name: str) -> Optional[pd.Series]:
        ...
        partial = self.dislike_df[
            self.dislike_df["dislike_name"]
            .str.lower()
            .str.contains(dislike_name.lower(), regex=False)
        ]
    ...
    def filter(
        self, user_dislikes: List[str], input_foods_df: Optional[pd.DataFrame] = None
    ) -> Tuple[pd.DataFrame, Dict]:

        base_df = (
            input_foods_df.copy() if input_foods_df is not None else self.food_df.copy()
        )
        ...
        for dislike in user_dislikes:
            ...
            if dislike_lower in self.CATEGORY_FILTERS:
                ...
                mask = (
                    safe_df["category"]
                    .str.lower()
                    .apply(lambda c: any(kw in c for kw in cat_keywords))
                )
            ...
            if rules is not None:
                ...
                if food_ids:
                    removed_names += safe_df[safe_df["food_id"].isin(food_ids)][
                        "food_name"
                    ].tolist()
                ...
                if cat_str and cat_str != "nan":
                    for cat in [
                        c.strip().lower() for c in cat_str.split(",") if c.strip()
                    ]:
                        cat_mask = (
                            safe_df["category"]
                            .str.lower()
                ...
                filter_log.append(f"[WARNING] '{dislike}' not in database — skipped")
