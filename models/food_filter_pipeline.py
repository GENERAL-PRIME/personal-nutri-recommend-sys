"""
models/food_filter_pipeline.py  v4.0
Master pipeline: chains AllergyFilter → DiseaseFilter → DislikeFilter.
Properly enforces all dietary preferences: none/vegetarian/vegan/jain/halal.
"""
...
class FoodFilteringPipeline:
    ...
    def run(self, user_profile: Dict) -> Dict:
        ...
        # ── Enforce dietary preferences through the dislike filter ────────────
        ...
        if diet_pref == "jain" and "jain diet" not in dl:
            ...
            # Halal: remove pork/alcohol; allow all other meats
            if "pork" not in dl and "no pork" not in dl:
                dislikes = ["No Pork"] + dislikes

        log = [
            f"Pipeline started — user: {user_id}",
            f"  Diet pref: {diet_pref or 'none (non-veg)'}",
        ...
        # Determine flags
        is_veg  = diet_pref in ("vegetarian","vegan") or any("vegetarian" in d.lower() for d in dislikes)
        is_vegan= diet_pref == "vegan" or any("vegan" == d.lower() for d in dislikes)
        is_nonveg = diet_pref in ("none","halal","") and not is_veg
        ...
        rec_ctx = {
            ...
            "is_vegan":            is_vegan,
            "is_halal":            diet_pref == "halal",
            "is_jain":             diet_pref == "jain",
            "is_non_veg":          is_nonveg,
    ...
    def _protein_target(self, diseases, profile):
        try:    w = float(profile.get("weight_kg", 70))
        except: w = 70.0
        for d in diseases:
    ...
    def save_output(self, result: Dict, label: str = "") -> dict:
        ...
        return {"json": json_file, "csv": csv_file, "is_update": is_update,
                "created_at": created_at, "last_updated": now}
