...
def save_last_plan(user_id: str, plan_data: dict):
    """
    Store the last generated diet plan locally for quick retrieval on login.
    """
    registry = _load()
    if user_id in registry["users"]:
        registry["users"][user_id]["last_plan"] = plan_data
        registry["users"][user_id]["last_run"] = datetime.now().isoformat()
        _save(registry)


def get_last_plan(user_id: str) -> dict | None:
    """
    Retrieve the last generated diet plan for a user.
    """
    user = get_user(user_id)
    if user:
        return user.get("last_plan")
    return None
